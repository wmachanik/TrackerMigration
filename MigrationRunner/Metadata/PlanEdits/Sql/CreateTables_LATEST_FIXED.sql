...
-- [NextRoastDateByCityTbl] Class=Copy Target=NextPreparationDateByAreasTbl EmittedCols=7
...
GO
    ...
    (
        ...
        [GroupItemServiceTypeID] INT NULL,
        [ItemID] INT NULL,
...
GO
    ...
    (
        ...
        [ItemPrepTypeDesc] NVARCHAR(50) NULL,
...
GO
    ...
    (
        ...
        [ItemServiceTypeName] NVARCHAR(20) NULL,
...
-- Drop FKs referencing or owned by [NextPreparationDateByAreasTbl]
DECLARE @sql nvarchar(max) = N'';
SELECT @sql = @sql + N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(o.schema_id)) + N'.' + QUOTENAME(OBJECT_NAME(fk.parent_object_id)) + N' DROP CONSTRAINT ' + QUOTENAME(fk.name) + N';' + CHAR(13)
FROM sys.foreign_keys fk
JOIN sys.objects o ON fk.parent_object_id = o.object_id
WHERE fk.parent_object_id = OBJECT_ID(N'[NextPreparationDateByAreasTbl]') OR fk.referenced_object_id = OBJECT_ID(N'[NextPreparationDateByAreasTbl]');
IF LEN(@sql) > 0 EXEC sp_executesql @sql;
IF OBJECT_ID(N'[NextPreparationDateByAreasTbl]', N'U') IS NOT NULL DROP TABLE [NextPreparationDateByAreasTbl];
GO
    CREATE TABLE [NextPreparationDateByAreasTbl]
    (
