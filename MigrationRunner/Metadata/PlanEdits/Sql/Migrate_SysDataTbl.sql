...
[ID], [LastRecurringDate], [DoReccuringOrders], [DateLastPrepDateCalcd], [MinReminderDate], [GroupReferenceItemID], [InternalContactIDs]
)
SELECT
[ID], CAST(dbo.SafeDateConvert([LastReoccurringDate]) AS DATE) AS [LastRecurringDate], [DoReoccuringOrders] AS [DoReccuringOrders], CAST(dbo.SafeDateConvert([DateLastPrepDateCalcd]) AS DATE) AS [DateLastPrepDateCalcd], CAST(dbo.SafeDateConvert([MinReminderDate]) AS DATE) AS [MinReminderDate], [GroupItemTypeID] AS [GroupReferenceItemID], [InternalCustomerIds] AS [InternalContactIDs]
