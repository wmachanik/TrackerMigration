    ...
    internal static class DdlScriptGenerator
    {
            ...
            try
            {
                ...
                foreach (var kv in ctByName)
                {
                    ...
                    foreach (var fk in ct.ForeignKeys ?? new List<ForeignKeyDef>())
                    {
                        ...
                        var parentModel = model[tbl];
                        var refModel = model[fk.RefTable];

                        if (!parentModel.Columns.ContainsKey(fk.Column))
                        {
                            sb.AppendLine($"-- Skipped: parent column not in model for [{tbl}].[{fk.Column}] -> [{fk.RefTable}]");
                            skipped++;
                            continue;
                        }
                        ...
                        if (!string.IsNullOrWhiteSpace(refCol) && !refModel.Columns.ContainsKey(refCol))
                        {
                            var fallbackPk = (refCt?.PrimaryKey ?? new List<string>())
                                .FirstOrDefault(c => refModel.Columns.ContainsKey(c));

                            if (!string.IsNullOrWhiteSpace(fallbackPk))
                            {
                                sb.AppendLine($"-- Adjusted FK reference for [{tbl}].[{fk.Column}] -> [{fk.RefTable}]: [{refCol}] -> [{fallbackPk}]");
                                refCol = fallbackPk;
                            }
                        }
                        ...
                        if (!refModel.Columns.ContainsKey(refCol))
                        {
                            sb.AppendLine($"-- Skipped: referenced column not in model for [{tbl}].[{fk.Column}] -> [{fk.RefTable}].[{refCol}]");
                            skipped++;
                            continue;
                        }
                        ...
                        // Create FK without checking existing data to avoid failures when data isn't fully consistent yet
                        sb.AppendLine($"IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = N'{fkName}' AND parent_object_id = OBJECT_ID(N'[{tbl}]'))");
                        sb.AppendLine("BEGIN");
                        sb.AppendLine($"    ALTER TABLE [{tbl}] WITH NOCHECK ADD CONSTRAINT [{fkName}] FOREIGN KEY([{fk.Column}]) REFERENCES [{fk.RefTable}]([{refCol}]);");
                        sb.AppendLine("END");
                        sb.AppendLine("GO");
                        sb.AppendLine();
                        // Now attempt to validate the FK; if validation fails, emit a WARN but continue
                        sb.AppendLine("BEGIN TRY");
                        sb.AppendLine($"    ALTER TABLE [{tbl}] WITH CHECK CHECK CONSTRAINT [{fkName}];");
                        sb.AppendLine($"    PRINT N'OK: Checked constraint {fkName} on [{tbl}]';");
                        sb.AppendLine("END TRY");
                        sb.AppendLine("BEGIN CATCH");
                        sb.AppendLine($"    PRINT N'WARN: could not CHECK [{fkName}] on [{tbl}]: ' + ERROR_MESSAGE();");
        ...
        private static Dictionary<string, TargetTable> BuildTargetModel(string migrationsDir, out ConstraintsIndex constraints, out List<string> errors)
        {
            ...
            var constraintsPath = File.Exists(consPrimary) ? consPrimary
                                ...
                                : consPrimary;
            HashSet<string> excludeTables = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            ...
            if (File.Exists(constraintsPath))
            {
                try
                {
                    constraints = JsonConvert.DeserializeObject<ConstraintsIndex>(
                        File.ReadAllText(constraintsPath)
                    ) ?? new ConstraintsIndex();

                    // Populate exclude list safely
                    excludeTables = new HashSet<string>(
                        constraints.ExcludeTables ?? new List<string>(),
                        StringComparer.OrdinalIgnoreCase
                    );
                }
                                          ...
                                          .Where(f => !string.Equals(Path.GetFileName(f), "index.json", StringComparison.OrdinalIgnoreCase)))
            {
                ...
                // BLOCK excluded tables (source OR target)
                if (excludeTables.Contains(s.SourceTable) ||
                    (!string.IsNullOrWhiteSpace(singleTarget) && excludeTables.Contains(singleTarget)))
                {
                    continue;
                }
                ...
                if (!isNormalize)
                {
                    ...
                    var tt = GetOrAdd(target, singleTarget, excludeTables);
                ...
                else
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(n.HeaderTable))
                    {
                        ...
                        if (excludeTables.Contains(n.HeaderTable))
                            continue;

                        var th = GetOrAdd(target, n.HeaderTable,excludeTables);
                    ...
                    {
                        if (excludeTables.Contains(n.LineTable))
                            continue;

                        var tl = GetOrAdd(target, n.LineTable, excludeTables);
                        foreach (var a in actions
        ...
        private static TargetTable GetOrAdd(Dictionary<string, TargetTable> map,
            string name,HashSet<string> excludeTables)
        {
