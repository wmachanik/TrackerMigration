...
namespace MigrationRunner
{
    ...
    internal static class DdlScriptApplier
    {
        ...
        {
            return ApplyLatest(migrationsDir, connectionString, out logPath, DdlApplyScope.CreateTablesAndForeignKeys);
        }

        public static int ApplyLatest(string migrationsDir, string connectionString, out string logPath, DdlApplyScope scope)
        {
            ...
            // Prefer manually curated script when present (see DATETIME_TO_DATE_MIGRATION_CHECKLIST.md).
            var fixedCreate = Path.Combine(sqlDir, "CreateTables_LATEST_FIXED.sql");
            FileInfo create = null;
            if (File.Exists(fixedCreate))
            {
                create = new FileInfo(fixedCreate);
            }
            else
            {
                create = Directory.EnumerateFiles(sqlDir, "CreateTables_*.sql", SearchOption.TopDirectoryOnly)
                                  .Where(p => !p.EndsWith("_FIXED.sql", StringComparison.OrdinalIgnoreCase))
            ...
            if (scope != DdlApplyScope.ForeignKeysOnly && create != null) steps.Add(create);
            if (scope != DdlApplyScope.CreateTablesOnly && fks != null) steps.Add(fks);

            var sb = new StringBuilder();
            if (steps.Count == 0)
            {
                sb.AppendLine("No SQL files found to apply for scope " + scope + " in: " + sqlDir);
                File.WriteAllText(logPath, sb.ToString(), Encoding.UTF8);
                return 2;
            }

            sb.AppendLine("Applying DDL scripts to target DB (scope: " + scope + ")");
            if (create != null && create.Name.Equals("CreateTables_LATEST_FIXED.sql", StringComparison.OrdinalIgnoreCase))
            {
                sb.AppendLine("Using manually curated CreateTables_LATEST_FIXED.sql");
            }
            sb.AppendLine("Files:");
        ...
        private static int ExecuteBatches(SqlConnection conn, string script, StringBuilder log, string currentFile)
        {
            ...
            foreach (var part in parts.Select(p => p?.Trim()).Where(p => !string.IsNullOrWhiteSpace(p)))
            {
                using (var cmd = conn.CreateCommand())
                {
                    ...
                    catch (SqlException sex)
                    {
                        ...
                        // Continue with next batch instead of throwing so we can emit all FKs and logs
                        log.AppendLine("  ? Continuing after error in batch.");
                    }
                    catch (Exception ex)
                    {
                        hadErrors = true;
            ...
            // If any batch had errors, return -executed to indicate partial failures
            return hadErrors ? -executed : executed;
        }
