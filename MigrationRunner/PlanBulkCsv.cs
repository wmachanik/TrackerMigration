                    ...
                    if (File.Exists(columnsCsvPath))
                    {
                        ...
                        if (lines.Length > 1)
                        {
                            ...
                            for (int i = 1; i < lines.Length; i++)
                            {
                                ...
                                var srcColRaw = Get(cols, iSrcCol);
                                // Normalize placeholder tokens like "n/a" -> treat as no source
                                var srcCol = PlanCsvIO.NormalizeSourceToken(srcColRaw);
                                if (string.IsNullOrWhiteSpace(srcTbl)) continue;
                                ...
                                // Try to find existing ColumnPlan by Source; if source is placeholder/blank, try match by Target
                                var ca = (ColumnPlan)null;
                                if (!string.IsNullOrWhiteSpace(srcCol)) ca = s.Plan.ColumnActions.FirstOrDefault(a => string.Equals(a.Source, srcCol, StringComparison.OrdinalIgnoreCase));
                                if (ca == null && !string.IsNullOrWhiteSpace(tgtCol)) ca = s.Plan.ColumnActions.FirstOrDefault(a => string.Equals(a.Target, tgtCol, StringComparison.OrdinalIgnoreCase));
                                if (ca == null)
                                {
                                    ca = new ColumnPlan { Source = srcCol ?? "", Target = string.IsNullOrWhiteSpace(tgtCol) ? (srcCol ?? "") : tgtCol, Action = "Copy" };
