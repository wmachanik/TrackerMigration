            ...
            try
            {
                ...
                using (var acc = new OleDbConnection(accessConnString))
                {
                    ...
                    if (File.Exists(constraintsPath))
                    {
                        try
                        {
                            var constraints = JsonConvert.DeserializeObject<ConstraintsIndex>(
                        ...
                        {
                            log.Add("WARNING: Failed to load ExcludeTables: " + ex.Message);
                        }
                    }

                    var baseTables = jsonTableSet.Count > 0 ? jsonTableSet.ToList() : accessTables.ToList();

                    // FILTER OUT EXCLUDED TABLES
                    var tablesToStage = baseTables
                        .Where(t => !excludeTables.Contains(t))
                        .ToList();
                    ...
                    foreach (var src in tablesToStage.OrderBy(x => x, StringComparer.OrdinalIgnoreCase))
                    {
                        ...
                        try
                        {
                            ...
                            var liveAccessCols = GetAccessColumnNames(acc, srcTrim);
                            foreach (var liveCol in liveAccessCols)
                            {
                                if (!colNames.Contains(liveCol, StringComparer.OrdinalIgnoreCase))
                                    colNames.Add(liveCol);
                            }

                            if (colNames.Count == 0)
                                colNames = liveAccessCols;
                            if (colNames.Count == 0)
                            {
