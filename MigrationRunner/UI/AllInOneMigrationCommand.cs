...
namespace MigrationRunner.UI
{
    internal sealed class AllInOneMigrationCommand : IMenuCommand
    {
        ...
        private static void ReportObjectReferences(string migrationsDir, string sqlConnString, string[] terms)
        {
            if (terms == null || terms.Length == 0) return;
            Console.WriteLine("-- Searching database object definitions for suspicious identifiers...");
            using (var conn = new SqlConnection(sqlConnString))
            {
                conn.Open();
                foreach (var term in terms.Distinct().Where(t => !string.IsNullOrWhiteSpace(t)))
                {
                    try
                    {
                        using (var cmd = conn.CreateCommand())
                        {
                            ...
                            cmd.CommandText = @"SELECT  OBJECT_SCHEMA_NAME(m.object_id) AS SchemaName, OBJECT_NAME(m.object_id) AS ObjName, LEFT(m.definition, 2000) AS Snippet
FROM sys.sql_modules m
WHERE m.definition LIKE @p";
                            ...
                            using (var rdr = cmd.ExecuteReader())
                            {
                                ...
                                {
                                    found = true;
                                    var s = rdr[0] as string ?? "";
                                    var o = rdr[1] as string ?? "";
                                    var sn = rdr[2] as string ?? "";
                                    Console.WriteLine($"  OBJECT: {s}.{o} contains '{term}' -> snippet:\n{sn}");
                                }
                                if (!found) Console.WriteLine($"  No objects found containing '{term}'.");
                            }
                        }
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine($"  ERROR searching for '{term}': {ex.Message}");
                    }
                }
            }
        }

        private static void ReportFkOrphans(string migrationsDir, string sqlConnString)
        {
            var constraintsPath = Path.Combine(migrationsDir, "Metadata", "PlanEdits", "PlanConstraints.json");
            if (!File.Exists(constraintsPath))
            {
                Console.WriteLine("FK orphan report: PlanConstraints.json not found: " + constraintsPath);
                return;
            }

            var json = File.ReadAllText(constraintsPath);
            MigrationRunner.ConstraintsIndex index = null;
            try { index = Newtonsoft.Json.JsonConvert.DeserializeObject<MigrationRunner.ConstraintsIndex>(json); }
            catch (Exception ex) { Console.WriteLine("Failed to parse PlanConstraints.json: " + ex.Message); return; }

            Console.WriteLine("-- FK Orphan report (showing child table, FK column, parent table, missing count)");
            using (var conn = new System.Data.SqlClient.SqlConnection(sqlConnString))
            {
                conn.Open();
                foreach (var table in index.Tables ?? new System.Collections.Generic.List<MigrationRunner.ConstraintTable>())
                {
                    var child = table.Table;
                    if (table.ForeignKeys == null) continue;
                    foreach (var fk in table.ForeignKeys)
                    {
                        if (string.IsNullOrWhiteSpace(fk.Column) || string.IsNullOrWhiteSpace(fk.RefTable) || string.IsNullOrWhiteSpace(fk.RefColumn))
                            continue;
...
LEFT JOIN [{fk.RefTable}] p ON ISNULL(c.[{fk.Column}], N'') = ISNULL(p.[{fk.RefColumn}], N'')
WHERE c.[{fk.Column}] IS NOT NULL AND p.[{fk.RefColumn}] IS NULL";
                                cmd.CommandTimeout = 0;
                                var missing = Convert.ToInt32(cmd.ExecuteScalar() ?? 0);
                                if (missing > 0)
                                {
                                    Console.WriteLine($"  ORPHANS: {child}.{fk.Column} -> {fk.RefTable}.{fk.RefColumn}: {missing}");

                                    // Show sample missing values
                                    using (var sCmd = conn.CreateCommand())
                                    {
                                        sCmd.CommandText = $@"SELECT TOP 10 CONVERT(nvarchar(200), c.[{fk.Column}]) FROM [{child}] c LEFT JOIN [{fk.RefTable}] p ON ISNULL(c.[{fk.Column}], N'') = ISNULL(p.[{fk.RefColumn}], N'') WHERE c.[{fk.Column}] IS NOT NULL AND p.[{fk.RefColumn}] IS NULL";
                                        sCmd.CommandTimeout = 0;
                                        using (var rdr = sCmd.ExecuteReader())
                                        {
                                            var list = new System.Collections.Generic.List<string>();
                                            while (rdr.Read()) list.Add(rdr.IsDBNull(0) ? "NULL" : rdr.GetString(0));
                                            if (list.Count > 0) Console.WriteLine("    Sample missing values: " + string.Join(", ", list));
                                        }
                                    }
                                }
                            }
                        ...
                        {
                            Console.WriteLine($"  ERROR checking FK {child}.{fk.Column} -> {fk.RefTable}.{fk.RefColumn}: {ex.Message}");
                        }
                    }
                }
            }
...
IF LEN(@sql) > 0 EXEC sp_executesql @sql;";
            ...
            // B) (DEFERRED) FK generation is performed after data migration to avoid constraint-check failures
            Console.WriteLine("B) Generate FK script: deferred until after data migration (to avoid FK check failures)");
            ...
            string applyErrorLog = null;
            try
            {
                int rcApply = RunWithHeartbeat(() => DmlScriptApplier.ApplyLatest(_migrationsDir, sql, out applyLog, out applyErrorLog), "N) Apply DATA");
                Console.WriteLine("N) Apply DATA rc=" + rcApply + " log: " + applyLog);
                Console.WriteLine("N) Apply DATA error log: " + applyErrorLog);
                Tail(applyLog, 48);
                if (rcApply != 0)
                {
                    Console.WriteLine("Data migration reported issues.");
                    // Run object reference report to find database objects that mention suspect column names
                    try
                    {
                        ReportObjectReferences(_migrationsDir, sql, new[] { "ContactsItemsPredictedId", "ContactsItemsPredictedID" });
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine("Post-N) Failed to run object reference report: " + ex.Message);
                    }
                }
            }
            catch (Exception ex)
            ...
            // After data migration we should generate and apply foreign keys now that data is present
            try
            {
                var rcB2 = DdlScriptGenerator.GenerateForeignKeys(_migrationsDir, out var fkPath2);
                Console.WriteLine("Post-N) Generate FK script rc=" + rcB2 + " file: " + fkPath2);
                if (rcB2 == 0)
                {
                    string fkApplyLog = null;
                    int rcApplyFks = RunWithHeartbeat(() => DdlScriptApplier.ApplyLatest(_migrationsDir, sql, out fkApplyLog), "Post-N) Apply FK DDL");
                    Console.WriteLine("Post-N) Apply FK DDL rc=" + rcApplyFks + " log: " + fkApplyLog);
                    Tail(fkApplyLog, 32);
                    if (rcApplyFks != 0)
                    {
                        Console.WriteLine("WARNING: Applying FK DDL reported issues.");
                    }
                    // Run FK orphan report to surface missing parent rows
                    try
                    {
                        ReportFkOrphans(_migrationsDir, sql);
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine("Post-N) Failed to run FK orphan report: " + ex.Message);
                    }
                }
                else
                {
                    Console.WriteLine("WARNING: FK generation returned non-zero rc=" + rcB2);
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine("Post-N) Failed to generate/apply FK DDL: " + ex.Message);
            }
