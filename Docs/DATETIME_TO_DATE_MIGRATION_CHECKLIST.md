...
### Column renames (AccessSrc → dbo) — do not “fix” both sides to the same name

Some Access columns were renamed in `CreateTables_LATEST_FIXED.sql`. In `Migrate_*.sql`:

- **INSERT** column list = **dbo** names (from CreateTables)
- **SELECT** = **AccessSrc** names (from Access), with `AS [dboColumn]` when they differ

| AccessSrc column | dbo column (`CreateTables_LATEST_FIXED.sql`) |
|------------------|-----------------------------------------------|
...
The checklist table below still lists legacy spellings (`NextPreperationDate`) in places; the **canonical dbo names** are in `CreateTables_LATEST_FIXED.sql` (`NextPreparationDate`, `PreparationDate`).

**Will it break again?** Yes, if you:
- Change the SELECT to `[NextPreparationDate]` (that name is not in AccessSrc)
- Run menu **`>`** (regenerates and deletes all `Migrate_*.sql`)
- Change CreateTables column spelling without updating the matching INSERT list in the migrate script

Option **99** does **not** overwrite `Migrate_*.sql`.
